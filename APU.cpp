...
volatile unsigned char APU_Reg[ 0x20 ];

void __USER_ISR timer2isr(void);
...
float P1TuningWord_f;
unsigned int P1TuningWord;
...
void ApuInit()
{
    ...
    setIntVector(_TIMER_2_VECTOR, timer2isr);
...
void __USER_ISR timer2isr(void) {
    ...
    // Mixing
    //MixDac = (P1Dac + P2Dac + TrDac + NsDac) / 4;
    tempDACtotal = (P1Dac + P2Dac + TrDac + NsDac)/16 ;
    MixDac = (WORD)(tempDACtotal)/4;
    //elaspedTime = _CP0_GET_COUNT();
  ...
  switch( wAddr )
  {
    ...
    case 3:
      ...
      if (wAddr == 3)
      {
        ...
        // Reset sequencer
        P1Vol = 15;         // Added here - volume must be restarted every each load! (NYH)
        P1Lapse = 0;
        P1EnvSeq = P1EnvCnt;
        P1Sweep = 0;
        P1SwCnt = 0;
    ...
    case 7:
      ...
      if (wAddr == 7)
      {
        ...
        // Reset sequencer
        P2Vol = 15;         // Added here - volume must be restarted every each load! (NYH)
        P2Lapse = 0;
        P2EnvSeq = P2EnvCnt;
        P2Sweep = 0;
        P2SwCnt = 0;
    ...
    case 0xF:
      ...
      // Reset sequencer
      NsEnvSeq = NsEnvCnt;
      NsVol = 15;              // Added here - volume must be restarted every each load! (NYH)
      break;
