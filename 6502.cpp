...
extern unsigned short APU_Reg[0x20];

extern unsigned char joypad0_value;
...
void put6502memory(WORD addr, BYTE value)
{
    ...
    case 0x4000:    /*APU 声音处理 IO接口 扩展ROM*/
        ...
        {
          //return (apu_regbase[addr&0x1F]);  //暂不处理
		  //APU_Reg[addr&0x1f] = value;
		  ApuWrite(addr&0x7f, value);
        }
        if(addr == 0x4014)
        {
...
void exec6502(int timerTicks)
{
    ...
    {
		// add a 64-bit counter here and increment here!!
